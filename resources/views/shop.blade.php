...
<form id="frmFilter" method="GET" action="{{ route('shop.index') }}">
  ...
  <select id="sorting" name="sorting" class="...">
    <option value="default"   {{ $sorting=='default'?'selected':'' }}>Default</option>
    <option value="date"      {{ $sorting=='date'?'selected':'' }}>Newness</option>
    <option value="price"     {{ $sorting=='price'?'selected':'' }}>Price ↑</option>
    <option value="price-desc"{{ $sorting=='price-desc'?'selected':'' }}>Price ↓</option>
  </select>
</form>
...
$(function () {
    ...
    $('input[name="brands"]').on('change', function () {
        const brands = $('input[name="brands"]:checked')
            .map((_, el) => el.value)
            .get()
            .join(',');
